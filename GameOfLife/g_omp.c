...
int main(int argc,char *argv[]){
	...
	{
	#pragma omp parallel for private(i,j) collapse(2) shared(matrix,matrice)
	for(i = 1; i <=	numarLinii; i++)
	{
		for(j = 1; j <= numarColoane; j++)
		{
			
			int numarVeciniVii = 0;
			int numarVeciniMorti = 0;
	...
	//BORDARE
	#pragma omp parallel for private(i,j) collapse(2) shared(matrice)
	for(i = 0; i <= numarLinii + 1; i++)
	{
		for(j = 0; j <= numarColoane + 1; j++)
		{
	...
	//COPIERE
	#pragma omp parallel for private(i,j) collapse(2) shared(matrice,matrix)
	for(i = 0; i <= numarLinii + 1;i++)
	{
		for(j = 0; j <= numarColoane + 1 ; j++)
			{
